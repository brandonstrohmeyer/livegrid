...
export type DateTextCandidate = {
  text: string | null | undefined
  source: string
}

export type ResolvedEventDateRange = {
  start: Date | null
  end: Date | null
...
export type HODOrgEventListing = {
  eventUrl: string
...
function normalizeYear(rawYear: string | number | null | undefined, fallbackYear: number) {
...
function monthNameToIndex(value: string | null | undefined) {
...
export function parseDateRangeFromText(text: string, fallbackDate?: Date | null): DateRange | null {
  ...
  const textWithoutWeekdays = normalizeText(
    normalized.replace(new RegExp(`\\b${WEEKDAY_PATTERN}\\b,?`, 'gi'), ' ')
  )
  const fallbackYear = fallbackDate?.getUTCFullYear?.() ?? fallbackDate?.getFullYear?.() ?? new Date().getUTCFullYear()

  const repeatedMonthRegex = new RegExp(
    `\\b${MONTH_PATTERN}\\s+(\\d{1,2})(?:st|nd|rd|th)?\\s*-\\s*${MONTH_PATTERN}\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:[^0-9]+(\\d{4}))?`,
    'i'
  )
...
export function resolveEventDateRangeFromCandidates(
  candidates: DateTextCandidate[],
  options: { fallbackDate?: Date | null; allowFallbackDate?: boolean } = {}
): ResolvedEventDateRange {
...
function extractDateTextFromContext(text: string) {
  const normalized = normalizeText(text)
  const patterns = [
...
export function extractHodEventListingsFromOrg(html: string): HODOrgEventListing[] {
  const listings: HODOrgEventListing[] = []
